...
mongoose.connect('mongodb://192.168.50.30:27017/watchout_db')
  ...
  .catch(err => console.error('DB Connection Error (Ensure VM is set to Host-Only):', err));
...
async function importData() {
  try {
    // 1. Read the file we downloaded in Phase 1
    if (!fs.existsSync('movies.json')) {
        console.error("Error: 'movies.json' not found. Did you run Phase 1?");
        process.exit(1);
    }
